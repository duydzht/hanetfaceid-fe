...
const TABS = [
  {
    title: "Đại biểu Khách mời",
    type: Type.GUEST,
  },
  {
    title: "Đại biểu Đại hội",
    type: Type.DELEGATE,
  },
];
...
export default function Documents(props) {
  ...
  const [selectedType, setSelectedType] = useState(Type.GUEST);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setDocuments({
        ...
        guest: data.filter(({ attributes: { type } }) => type === Type.GUEST),
        delegate: data.filter(
          ({ attributes: { type } }) => type === Type.DELEGATE
        ),
      });
    } catch (error) {
  ...
  const data = useMemo(() => {
    const docs = orderBy(
      (documents?.all || []).concat(
        selectedType === Type.GUEST ? documents?.guest : documents?.delegate
      ),
      (item) => item?.attributes?.no,
      ["asc"]
    );
